...
from two_sample_utilities import xgboost_variable_importance, xgboost_roc_curve, xgboost_overtraining_check, nn_roc_curve, nn_overtraining_check
...
variables = ['B_PT', 'B_FDCHI2_OWNPV', 'B_DiraAngle',
             ...
             'K1_1270_FDCHI2_OWNPV',
             'piplus_ProbNNpi', 'piminus_ProbNNpi']

# Construct the arrays from the data
parray_sim = events_sim.arrays(variables + ['B_BKGCAT', 'K1_1270_M', 'B_M', 'B_ENDVERTEX_CHI2', 'muplus_isMuon', 'muminus_isMuon'], library = 'pd').query('B_PT > 2000 & B_DiraAngle < 0.02 &  K_ProbNNk > 0.1 & piplus_ProbNNpi > 0.1 & piminus_ProbNNpi > 0.1 & B_ENDVERTEX_CHI2 < 25 & K1_1270_M < 2400 & muplus_isMuon == True & muminus_isMuon == True')
parray_real = events.arrays(variables + ['B_M'], library = 'pd').query('B_M > 5800')
...
parray_sim.drop(['B_BKGCAT', 'B_ENDVERTEX_CHI2', 'K1_1270_M', 'muminus_isMuon', 'muplus_isMuon'], axis=1, inplace=True)
...
parray.drop(['B_M'], axis=1, inplace=True)
...
def optimiser(n_estimators, max_depth, learning_rate, n_iter, test_size, colsample_bytree=0.5, multicollinearity=True):
    ...
    for score in scoring:
        if score == 'ROC AUC':
            evaluation_test[score] = scoring[score](Y_test, probabilities_test)
        else:
    ...
    for score in scoring:
        if score == 'ROC AUC':
            evaluation_train[score] = scoring[score](Y_train, probabilities_train)
        else:
    ...
    xgboost_roc_curve( alg, X_test, Y_test, X_train, Y_train, filename='/home/physics/phunda/Figs/training_roc.pdf' )
    xgboost_overtraining_check( alg, X_train, Y_train, X_test, Y_test, filename='/home/physics/phunda/Figs/training_overtraining.pdf' )
    ...
    roc = roc_curve(Y_test.detach().numpy(), Y_test_pred_proba.detach().numpy())
    roc_train = roc_curve(Y_train.detach().numpy(), Y_train_pred_proba.detach().numpy())
    plt.plot(roc[0], roc[1], color='r', label='Test')
    plt.plot(roc_train[0], roc_train[1], color='b', label='Train')
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)
...
# Save the models
with open('XGBoost_5.pkl', 'wb') as f:
    pickle.dump(xgb, f)

with open('Deep_NN_5.pkl', 'wb') as f:
    pickle.dump(deep, f)
